...
async function fetchList() {
  try {
    ...
    blogList.value = (res && res.data) ? res.data : (Array.isArray(res) ? res : [])
  } catch {
    blogList.value = []
  }
}
...
async function deleteDoc(id) {
  ...
  try {
    ...
    blogList.value = blogList.value.filter(d => d.id !== id)
  } catch (e) {
...
async function toggleLike(docId) {
  ...
  try {
    ...
    const idx = blogList.value.findIndex(b => b.id === docId)
    if (idx !== -1) blogList.value[idx].likeCount = d.likeCount
  } catch (e) {
    if (e.status === 401) showAuth.value = true
  }
}
...
<template>
  <div class="blog-page">
    ...
    <div v-if="showList" class="blog-list-view">
      ...
      <div class="doc-grid">
        <div v-for="doc in blogList" :key="doc.id" class="doc-item glass-card">
          <div class="doc-thumb">
            <img v-if="doc.coverImage" :src="doc.coverImage" :alt="doc.title" @error="$event.target.style.display='none'" />
            <span v-else class="thumb-placeholder">📝</span>
          </div>
          <RouterLink :to="{ path: '/blog', query: { id: doc.id } }" class="doc-title-link">
            <h3>{{ doc.title }}</h3>
          </RouterLink>
...
.doc-item h3:hover { color: #38bdf8; }
...
.back-to-top:hover { background: rgba(14, 165, 233, 0.6); transform: scale(1.05); }
...
.like-btn-big:hover { background: rgba(255, 255, 255, 0.15); }
.like-btn-big.liked {
...
/* 日间模式 */
[data-theme="day"] .blog-list-view h1,
[data-theme="day"] .doc-header h1,
[data-theme="day"] .doc-item h3 {
  color: #7dd3fc;
}
</style>
