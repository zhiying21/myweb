...
import { blogList } from '@/data/documents'

const route = useRoute()
const docPath = computed(() => route.query.path || '')
const currentDoc = computed(() => blogList.find((d) => d.path === docPath.value))
...
<template>
  <div class="blog-page">
    ...
    <div v-if="showList" class="blog-list-view">
      ...
      <div class="doc-grid">
        <RouterLink
          v-for="(doc, i) in blogList"
          :key="doc.path"
          :to="{ path: '/blog', query: { path: doc.path } }"
          class="doc-item"
        >
          <div class="doc-thumb">
...
:global([data-theme="day"]) .blog-page {
  color: #222;
}
